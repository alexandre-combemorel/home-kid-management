...
export const Task = () => {
	...
	const listOfTaskFromMoment = useMemo(
		() => moment?.data.tasks ?? [],
		[moment],
	);
	...
	const areAllTaskValidated = useMemo(
		() =>
			taskValidated.length + taskSkipped.length ===
			listOfTaskFromMoment?.length,
			...
			if (areAllTaskValidated) {
				const taskAndActions = values.taskValidated
					...
					.reduce<PayloadActions>((acc, taskId) => {
						const listOfKeys = Object.keys(values).filter((key) =>
							key.startsWith(taskId),
						);

						acc[taskId] = listOfKeys.reduce<{ [key2: string]: boolean }>(
							(acc2, taskactionkey) => {
				...
				<Form methods={methods} errors={errors} onSubmit={onSubmit}>
					<Stack alignItems="center" gap={3}>
						<Submit disabled={!areAllTaskValidated}>
							Terminer ({taskValidated.length + taskSkipped.length}/
							{listOfTaskFromMoment.length}) 👍
						</Submit>
								...
								return (
									...
									>
										...
										>
											...
											{!isCurrentTaskSkipped ? (
												<FormActions
													isValidated={taskValidated.includes(
														`task_${task.documentId}`,
													)}
													task={task}
													onValidate={() => {
